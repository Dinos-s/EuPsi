<?php
    ...
    if (!empty($_GET['search'])) {
        $data = $_GET['search'];
        $query = "SELECT marcados.*, pacientes.nome AS nome_paciente, psicologos.nome AS nome_psicologo
                  FROM marcados
                  JOIN pacientes ON marcados.id_paciente = pacientes.id
                  JOIN psicologos ON marcados.id_psicologo = psicologos.id
                  WHERE pacientes.nome LIKE '%$data%' OR psicologos.nome LIKE '%$data%'
                  ORDER BY marcados.id";
    } else {
        $query = "SELECT marcados.*, pacientes.nome AS nome_paciente, psicologos.nome AS nome_psicologo
                  FROM marcados
                  JOIN pacientes ON marcados.id_paciente = pacientes.id
...
<body>
    ...
    <main>
        <div class="pesquisa">
            <input type="text" id="pesquisa" placeholder="Pesquise por um paciente ou por um psicólogo">
        ...
        <nav>
            ...
            <ul class="menu-admim">
                ...
                <li><a class="active" href="./adminMarcados.php">Pacientes Marcados</a></li>
            </ul>
        </nav>
            ...
            <div class="psicologos">
                ...
                <div id="psicologos-list">
                    <table>
                        ...
                        <tbody>
                            <?php while ($marcados = mysqli_fetch_assoc($result)) {
                                print_r ($marcados);
                                echo "<tr>";
                                echo "<td>" . $marcados['nome_paciente'] . "</td>";
                                echo "<td>" . $marcados['nome_psicologo'] . "</td>";
                                echo "<td>" . $marcados['horario'] . "</td>";
                                echo "</tr>";
                            }
                            ?>
                        </tbody>
