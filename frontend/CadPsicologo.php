<?php 
    ...
    if (isset($_POST['submit'])) {
        ...
        if ($senha != $senhaRept) {
            echo 'As senha são Imcompatívei, verifique as senhas';
        } else {
            if (!validaCPF($cpf)) {
                echo "CPF Inválido";
            } else {
                ...
                $result = mysqli_query($conexao, "INSERT INTO psicologos (nome, crp, cpf, email, telefone, senha, createdAt, updatedAt) VALUES ('$nome', '$crp', '$cpf', '$email', '$telefone', '$senhaHash', '$dataAtual', '$dataAtual')");
